...
function revealing_bivariate_factorization_ff(F::T) where {T}
    R = parent(F)
...
function bivariate_factorization_ff_with_known_univariate(F, fi, Si)
    t, u = gens(parent(F))
    b = leading_coefficient_in(F, t)
    m = u
    n, d = degree(F, t), degree(F, u)
    l = d + 1 + degree(b, u)
    # recombine the factors for free
