...
mutable struct PrimesBenOrTiwari{Ring}
    ...
    ps::Vector{UInt}
    function PrimesBenOrTiwari(ring::Ring, T::Integer) where {Ring}
        @assert T >= 0
        K = base_ring(ring)
        n = nvars(ring)
...
function factor_exponents(mi::Vector{T}, n::Integer) where {T}
    exps = map(m -> Primes.factor(Dict, Int(data(m))), mi)
    monoms = Vector{Vector{Int}}(undef, length(exps))
    for (i, e) in enumerate(exps)
        monoms[i] = zeros(Int, n)
        for (p, e) in e
            if !haskey(_prime_to_idx, p)
                for j in 1:length(exps)
                    monoms[j] = zeros(Int, n)
                end
                return monoms
            end
            idx = _prime_to_idx[p]
